...
| Iteration 1 | Created portable workflow instructions and removed organization-specific assumptions. | The workflow should be reusable across agents, providers, and repos. | `workflow/AGENT.md`, `phases/phase-1-generic-workflow-design.md` | Keep `workflow/AGENT.md` as the source of truth and avoid tool-specific skill formats. |
...
| Iteration 10 | Prepared public release packaging with `package.json`, MIT license, `npx` installer, installer tests, public release phase doc, and updated README/reproduction instructions. | Clean-environment use needs a simple way to install agent-readable workflow files without making daily propagation a CLI workflow. | `package.json`, `bin/propagate-env.mjs`, `LICENSE`, `phases/phase-10-public-release.md`, `npm test`, `npm run pack:check` | Use `npx propagate-env install` as bootstrap. Keep propagation, setup, memory updates, and cleanup plain-English driven inside the user's agent. |
